...
Indices GraphModelStorage::getRandomNodeIds(int32_t size) {
    torch::TensorOptions ind_opts = torch::TensorOptions().dtype(torch::kInt32).device(storage_ptrs_.edges->device_);
...
void GraphModelStorage::initializeInMemorySubGraph(torch::Tensor buffer_state) {
    if (useInMemorySubGraph()) {
        ...
        buffer_state = buffer_state.to(torch::kInt32);
        ...
        torch::Tensor in_mem_edge_bucket_ids = torch::zeros({num_edge_buckets_in_mem}, torch::kInt32);
        torch::Tensor in_mem_edge_bucket_sizes = torch::zeros({num_edge_buckets_in_mem}, torch::kInt32);
        torch::Tensor global_edge_bucket_starts = torch::zeros({num_edge_buckets_in_mem}, torch::kInt32);
        ...
        torch::Tensor edge_bucket_sizes = torch::from_blob(edge_bucket_sizes_.data(), {(int)edge_bucket_sizes_.size()}, torch::kInt32);
...
#pragma omp parallel for
        ...
        current_subgraph_state_->all_in_memory_edges_ = torch::empty({total_size, storage_ptrs_.edges->dim1_size_}, torch::kInt32);
...
#pragma omp parallel for
        ...
        mapped_edges = mapped_edges.to(torch::kInt32);
        mapped_edges_dst_sort = mapped_edges_dst_sort.to(torch::kInt32);
        ...
        if (storage_ptrs_.train_edges != nullptr) {
            src_sort = storage_ptrs_.train_edges->range(0, storage_ptrs_.train_edges->getDim0()).to(torch::kInt32);
            dst_sort = storage_ptrs_.train_edges->range(0, storage_ptrs_.train_edges->getDim0()).to(torch::kInt32);
        } else {
            src_sort = storage_ptrs_.edges->range(0, storage_ptrs_.edges->getDim0()).to(torch::kInt32);
            dst_sort = storage_ptrs_.edges->range(0, storage_ptrs_.edges->getDim0()).to(torch::kInt32);
        }

        src_sort = src_sort.index_select(0, torch::argsort(src_sort.select(1, 0))).to(torch::kInt32);
        dst_sort = dst_sort.index_select(0, torch::argsort(dst_sort.select(1, -1))).to(torch::kInt32);
...
#pragma omp parallel for
    ...
    torch::Tensor new_edge_bucket_ids = torch::zeros({num_new_edge_buckets}, torch::kInt32);
    torch::Tensor new_edge_bucket_sizes = torch::zeros({num_new_edge_buckets}, torch::kInt32);
    torch::Tensor new_global_edge_bucket_starts = torch::zeros({num_new_edge_buckets}, torch::kInt32);
...
#pragma omp parallel for
    ...
    torch::Tensor src_ids_order = torch::zeros({num_edge_buckets_in_mem}, torch::kInt32);
...
#pragma omp parallel for
    ...
    torch::Tensor new_all_in_memory_edges = torch::empty({total_size, storage_ptrs_.edges->dim1_size_}, torch::kInt32);
