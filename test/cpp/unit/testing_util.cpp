...
torch::Tensor getRandTensor(int dim0_size, int dim1_size, torch::Dtype dtype, int max_val) {
    if (dtype == torch::kInt32 || dtype == torch::kInt32) {
...
void sortWithinEdgeBuckets(torch::Tensor &rand_tensor, vector<int32_t> &edge_bucket_sizes, int sort_dim) {
    int32_t offset = 0;
...
vector<int32_t> partitionEdges(torch::Tensor &edges, int num_partitions, int num_nodes) {
    ...
    int count = 1;
    vector<int32_t> partition_sizes_;
